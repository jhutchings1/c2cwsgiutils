...
def _init() -> None:
    ...
    if url:
        ...
        _master = redis.Redis.from_url(url, socket_timeout=socket_timeout)
        _slave = _master
    LOG.info("No Redis configuration found, use %s or %s to configure it", REDIS_URL_KEY, REDIS_SENTINELS_KEY)
